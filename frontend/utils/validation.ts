...
export const registerSchema = z.object({
  firstName: z.string().min(2, 'First name must be at least 2 characters').regex(/^[a-zA-Z\s'.\-]+$/, 'First name can only contain letters and special characters (no numbers)'),
  lastName: z.string().min(2, 'Last name must be at least 2 characters').regex(/^[a-zA-Z\s'.\-]+$/, 'Last name can only contain letters and special characters (no numbers)'),
